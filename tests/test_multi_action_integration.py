...
class TestMultiActionContextPassing:
    ...
    @pytest.mark.asyncio
    async def test_web_search_to_note_with_use_results_from(self, mock_mcp_client, mock_llm_client):
        """Test web search followed by note creation using use_results_from field"""
        # Setup web agent
        web_agent = WebAgent(mcp_client=mock_mcp_client)
        ...
        mock_mcp_client.call.side_effect = [
            # Search call
            {
                "status": "ok",
                "result": [
                    {"title": "미 증시 상승", "url": "http://example.com/1"},
                    {"title": "나스닥 급등", "url": "http://example.com/2"}
                ]
            },
            # Fetch first result
            {
                "status": "ok",
                "result": {"text": "미국 증시는 최근 상승세를 보이고 있습니다."}
            },
            # Fetch second result
            {
                "status": "ok",
                "result": {"text": "나스닥 지수가 급등했습니다."}
            }
        ]
        
        # Mock LLM summarization
        search_summary = "미국 증시 최근 근황:\n1. 미국 증시는 최근 상승세를 보이고 있습니다.\n2. 나스닥 지수가 급등했습니다."
        with patch.object(web_agent, '_summarize_with_llm', return_value=search_summary):
            web_params = {"intent": "web_search", "query": "미 증시 최근 근황"}
            web_result = await web_agent.handle(web_params)
            assert web_result["status"] == "ok"
            assert "summary" in web_result["result"]
        ...
        # Mock title generation
        with patch.object(note_agent, '_generate_title', return_value="미 증시 최근 근황"):
            # Mock note creation
            note_result = {"status": "ok", "result": "Note created", "message": "Note saved to Notion"}
            note_mock_mcp.call.return_value = note_result
            
            # Execute note creation with previous results (simulating use_results_from: [1])
            note_params = {
                "intent": "write_note",
                "text": "",  # Empty text, should use previous results
                "previous_results": [
                    {"action": 1, "intent": "web_search", "agent": "WebAgent", "result": web_result}
                ]
            }
            ...
            assert result["status"] == "ok"
            # Verify that notion_notes.write was called with the search result
            call_args = note_mock_mcp.call.call_args
            assert call_args[0][0] == "notion_notes"
            assert call_args[0][1] == "write"
            note_data = call_args[0][2]
            assert note_data["title"] == "미 증시 최근 근황"
