...
**스마트 정보 수집** ⭐ NEW:
- **"테슬라 최근 근황 메모해줘"** - 자동으로 웹 검색 후 메모 저장
- **"애플 주가 기록"** - 실시간 주가 검색 후 메모 저장
- **"미 증시 현황 정리해서 노트에 저장"** - 증시 정보 검색 후 요약하여 저장
- **"비트코인 시세 메모"** - 암호화폐 시세 검색 후 저장

**키워드 없이도 자동 검색**: "검색", "찾아" 같은 명시적 키워드 없이도 외부 정보가 필요한 경우 자동으로 웹 검색을 수행합니다.

**컨텍스트 전달**: 이전 액션의 결과가 다음 액션에 자동으로 전달됩니다. 웹 검색 결과는 메모나 일정의 내용으로 **완전히 대체**되어 저장됩니다 (기존 텍스트는 무시).
...
- **총 185개 테스트 모두 통과** ✅
- Parser 테스트: 25개 (외부/내부 정보 구분 테스트 포함)
- Agent 테스트: 33개
- MCP Tools 테스트: 25개
- Router 테스트: 23개
- Notion 통합 테스트: 13개
- E2E 통합 테스트: 18개
- Multi-Action 통합 테스트: 7개 (컨텍스트 전달 검증)
- API 서버 테스트: 17개
- 세션 관리 테스트: 20개
